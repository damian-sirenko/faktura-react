...
// ⛔️ Видалено імпорт дублюючого маршруту списку інвойсів
const { createInvoiceListPDF } = require("./invoices.list.pdf.js");

/* ===== 🔽 ДОДАНО для PDF-списку інвойсів ===== */
...
// доступ до MySQL (той самий pool, який ініціалізується в server/db.js)
...
// DEV-режим: можливість відключити авторизацію для локальної розробки
...
// works both locally and on hosting
...
// [CORS-SIMPLE] глобальний CORS до будь-яких роутів/гвардів
...
// fallback: гарантуємо ACAO/credentials на всіх шляхах, якщо origin у allow
app.use((req, res, next) => {
  const o = req.headers.origin;
  if (o && allow.has(o)) {
    res.header("Access-Control-Allow-Origin", o);
...
// cookies: читаємо або AUTH_COOKIE_SECURE, або COOKIE_SECURE
...
const cookieSecure = cookieSecureFlag === "true"; // true на HTTPS, false локально
...
// __tools-debug: показати звідки і що віддаємо
app.get("/__tools-debug", (_req, res) => {
  const pathMod = require("path");
  const fsMod = require("fs");
  const ROOT_LOCAL = __dirname;
  const TOOLS_PATH = pathMod.join(ROOT_LOCAL, "data", "tools.json");
  let stat = null,
    exists = fsMod.existsSync(TOOLS_PATH);
  try {
    stat = exists ? fsMod.statSync(TOOLS_PATH) : null;
  } catch {}
  let sample = null;
  try {
    sample = exists ? JSON.parse(fsMod.readFileSync(TOOLS_PATH, "utf8")) : null;
  } catch {}
  res.json({
    file: TOOLS_PATH,
    exists,
...
// [АНКОР] // ⬇︎ вставити відразу після створення app = express() та базових app.use(...)
app.use((req, res, next) => {
...
// === ➕ ДОДАНО: пошук і публічний шлях до "статичного підпису працівника" ===
...
function ensureStaticStaffSigPublic() {
  ...
  // якщо немає файлу — створимо просту «порожню» png, щоб ендпоінт не падав
...
app.use("/", signQueueFileRouter);

// server.js — confirm-гард для небезпечних дій
...
// глушилка для запиту Chrome DevTools
app.get("/.well-known/appspecific/com.chrome.devtools.json", (_req, res) => {
  // нічого не віддаємо, просто 204
  res.status(204).end();
});

// діагностика інвойсів
...
app.post("/__test-export", (req, res) => {
...
app.get("/generated/:filename", authGuard, async (req, res) => {
  ...
  try {
    // 1. спроба витягнути інвойс з БД за filename
    ...
    // 2. якщо не знайшли в БД — шукаємо фізично
...
app.use("/generated", authGuard, express.static(GENERATED_DIR));
...
app.get("/clients", async (_req, res) => {
  try {
    const rows = await clientsRepo.getAllClients(); // <-- тепер ВСІ, і archived=1 теж
    res.json(rows);
  } catch (err) {
...
// server.js — картотека клієнтів PDF
...
app.put("/invoices/:number", async (req, res) => {
  try {
    ...
    // якщо прилетів тільки статус — оновлюємо лише його, без чіпання інших полів
    const isStatusOnly =
...
app.post("/save-invoices", async (req, res) => {
  ...
  try {
    ...
    // додатково — автогенерація PDF, щоб файл існував у /generated
    setImmediate(async () => {
      for (const inv of invoices) {
        try {
          const fname =
...
app.get("/download-invoice/:filename", async (req, res) => {
  ...
  try {
    // 1. шукаємо інвойс у БД по імені файлу
    const dbRows = await invoicesRepo.queryInvoiceByFilename(fn);

    if (dbRows && dbRows.length) {
      const row = dbRows[0];

      // привести формат рядка з БД до формату, який очікує ensurePdfForInvoice()
      const invLikeOld = {
        number: row.number,
        client: row.clientName,
        issueDate: row.issueDate,
    ...
    // 2. fallback: шукаємо фізичний PDF файл на диску
    const p = findGeneratedFileDeep(fn);
    if (p && fs.existsSync(p)) {
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader("Content-Disposition", `inline; filename="${fn}"`);
...
// НОВЕ: PDF-список вибраних інвойсів
...
// Download multiple PDF as ZIP — перегенерація з БД перед упаковкою
app.post("/download-multiple", async (req, res) => {
  try {
    ...
    // підготуємо абсолютні шляхи, гарантовано свіжі
    const pathsToZip = [];
    for (const filename of files) {
      const safe = path.basename(String(filename || ""));
      // якщо є в БД — перегенерувати актуальний PDF і взяти його шлях
      ...
      if (dbRows && dbRows.length) {
        ...
        // ensurePdfForInvoice всередині виставляє amount_in_words пустим → PDFKit випише словами
      ...
      // fallback: фізичний файл з диска (якщо нема в БД)
...
// ЗАМІНИ ВЕСЬ БЛОК ФУНКЦІЇ dbListProtocolsWithEntriesLite НА НИЖЧЕ
async function dbListProtocolsWithEntriesLite() {
  ...
  for (const r of rows) {
    ...
    byProto.get(r.protocolId).entries.push({
      ...
      tools: [], // для списку не потрібно; у view підтягується повний протокол
...
// server.js — заміна функції parseJSON
function parseJSON(val, fallback) {
  try {
    ...
    // якщо драйвер MySQL (mysql2) вже розпарсив JSON-колонку:
...
function nowForSQL() {
  // Локальний час сервера у форматі "YYYY-MM-DD HH:MM:SS"
  const d = new Date();
...
async function dbGetProtocolFull(clientId, month) {
  ...
  let entriesRows = [];
  // 1) максимальний набір полів (нова схема)
  const fullSelect = `
      SELECT
        e.id                 AS entryId,
        e.date               AS date,
  ...
  `;
  // 2) середній набір полів (частково нова схема)
  const midSelect = `
      SELECT
        e.id               AS entryId,
        e.date             AS date,
      ...
      ORDER BY e.date ASC, e.id ASC
  `;
  // 3) мінімальна сумісність (стара схема без JSON-колонок)
  const minSelect = `
      SELECT
        e.id       AS entryId,
        e.date     AS date,
  ...
  const entries = entriesRows.map((r) => {
    const tools = r.hasOwnProperty("tools_json")
      ...
      : [];
    // підписи: з JSON, або зі старих колонок, або порожньо
    ...
    // блок повернення: якщо колонок немає — залишаємо null/порожньо
    const ret = {
...
async function dbUpsertEntry({ protocolId, index, newEntry }) {
  // 1. Дістаємо всі рядки протоколу, щоб знати id запису з таким index
  const rows = await sql(
    `
      SELECT
        e.id
  ...
  if (isUpdate) {
    ...
    // спроба ПОВНОГО UPDATE (нова схема)
    const fullSQL = `
    ...
    try {
      ...
      // fallback: старі колонки, які точно існують у твоїй таблиці зараз
      await sql(
        `
          UPDATE protocol_entries
          SET
  ...
  // INSERT (append у кінець протоколу)
  const fullInsertSQL = `
...
// утиліти, які залишаються спільними
...
function normalizeToolsArray(raw) {
  // Приводить будь-який формат до [{ name: "...", count: number }]
...
// server.cjs — ДОДАЙ ЦЕЙ МАРШРУТ ZIP ДЛЯ ВИБРАНИХ ПРОТОКОЛІВ
app.post("/protocols/zip", async (req, res) => {
  try {
    ...
    // використовуємо локальний HTTP виклик до існуючого PDF-роуту
...
/* ---- Додавання / редагування одного запису протоколу ---- */
app.post("/protocols/:clientId/:month", async (req, res) => {
  try {
    ...
    // signatures → файли на диск як раніше
    ...
    // queue автогенерація з логістики клієнта
    ...
    if (queue.courierPending) {
      // кур'єр: проставляємо courierPlannedDate
      const plannedRaw =
...
app.post("/protocols/:clientId/:month/:index/sign", async (req, res) => {
  try {
    ...
    if (staffDataURL !== undefined && staffDataURL !== null) {
      ...
      // гарантовано перевіряємо/оновлюємо файл щоразу
    ...
    // якщо обидві сторони підписані в transfer і return → чергу гасимо
...
/* ---- PATCH по запису з можливим переносом між місяцями ---- */
app.patch("/protocols/:clientId/:month/:index", async (req, res) => {
  try {
    ...
    if (movingToAnotherMonth) {
      // 1. видаляємо зі старого місяця
      await dbDeleteEntryByIndex(clientId, srcMonth, idx);
      // 2. додаємо у кінець нового
 ...
 * SIGN QUEUE API (DB)
 * ----------------------------------------------------------- */
app.get("/sign-queue-db", async (req, res) => {
  try {
...
// DEBUG: протоколи — коротка перевірка вмісту БД
...
// DEBUG: показати повний протокол, як його бачить бекенд
...
(() => {
  ...
  if (FRONT_DIR) {
    ...
    // SPA fallback без '*': загальний middleware
    app.use((req, res, next) => {
    ...
    // dev редірект на Vite без '*'
    app.use((req, res, next) => {
      if (req.method !== "GET") return next();
      if (API_PREFIXES.some((p) => req.path.startsWith(p))) return next();
      // //       res.redirect(302, `http://// 127.0.0.1:5173${req.originalUrl}`);
    });
  }
})();
initDB().catch((e) => {
